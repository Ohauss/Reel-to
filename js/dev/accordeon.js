import { d as disableScroll, e as enableScroll } from "./stepper.min.js";
var PipsMode;
(function(PipsMode2) {
...
const priceRadios = document.querySelectorAll(".prices__input");
const MIN = parseFloat(minInput == null ? void 0 : minInput.getAttribute("min")) || 0;
const MAX = parseFloat(maxInput == null ? void 0 : maxInput.getAttribute("max")) || 1e3;
const triggerFilterUpdate = () => {
  document.dispatchEvent(
    new CustomEvent("priceChange", {
      detail: {
        min: parseInt(minInput.value) || MIN,
        max: parseInt(maxInput.value) || MAX
      }
    })
  );
};
...
minInput.addEventListener("change", () => {
  const min = Math.max(MIN, Number(minInput.value) || MIN);
  const max = Number(maxInput.value) || MAX;
  pricerange.noUiSlider.set([min, Math.max(min, max)]);
  triggerFilterUpdate();
});
maxInput.addEventListener("change", () => {
  const min = Number(minInput.value) || MIN;
  const max = Math.min(MAX, Number(maxInput.value) || MAX);
  pricerange.noUiSlider.set([Math.min(min, max), max]);
  triggerFilterUpdate();
});
priceRadios.forEach((radio) => {
  radio.addEventListener("change", () => {
    const min = Number(radio.dataset.min);
    const max = Number(radio.dataset.max);
    minInput.value = min;
    maxInput.value = max;
...
function syncRadios(min, max) {
  let matched = false;
  priceRadios.forEach((radio) => {
    const rMin = Number(radio.dataset.min);
    const rMax = Number(radio.dataset.max);
...
document.addEventListener("DOMContentLoaded", () => {
  const catalogMobileFilters = document.querySelector(
    ".catalog__left--mobile-fixed"
  );
  const catalogMobileOpenFilters = document.querySelector(
    ".catalog__mobile-open-filters"
  );
  const catalogMobileFilterClose = document.querySelector(
    ".catalog__mobile-filter-close"
  );
  const catalogOverlay = document.querySelector(".overlay");
  const openFilter = () => {
    catalogMobileFilters.classList.add("catalog__left--mobile-visible");
    catalogOverlay == null ? void 0 : catalogOverlay.classList.add("overlay--visible");
    disableScroll();
  };
  const closeFilter = () => {
    catalogMobileFilters.classList.remove("catalog__left--mobile-visible");
    catalogOverlay == null ? void 0 : catalogOverlay.classList.remove("overlay--visible");
    enableScroll();
  };
  if (catalogMobileFilters && catalogMobileOpenFilters && catalogMobileFilterClose) {
    catalogMobileOpenFilters.addEventListener("click", openFilter);
    catalogMobileFilterClose.addEventListener("click", closeFilter);
    catalogOverlay == null ? void 0 : catalogOverlay.addEventListener("click", closeFilter);
  }
});
document.addEventListener("DOMContentLoaded", () => {
  ...
  customSelects.forEach((select) => {
    ...
    const chooses = Array.from(
      select.querySelectorAll(".custom-select__choose")
    );
    let currentIndex = chooses.findIndex(
      (el) => el.classList.contains("custom-select__choose--active")
    );
    if (currentIndex === -1) currentIndex = 0;
    const toggle = (show) => {
      var _a;
      const willShow = show ?? !dropdown.classList.contains("visible");
      dropdown.classList.toggle("visible", willShow);
      btn.setAttribute("aria-expanded", willShow ? "true" : "false");
      if (willShow) {
        (_a = chooses[currentIndex]) == null ? void 0 : _a.focus();
      }
    };
    const updateSelection = (index, silent = false) => {
      if (index < 0 || index >= chooses.length) return;
      const text = chooses[index].textContent.trim();
      const sortType = chooses[index].dataset.sort;
      chooses.forEach((el) => {
        el.classList.remove("custom-select__choose--active");
        el.setAttribute("aria-selected", "false");
      });
      chooses[index].classList.add("custom-select__choose--active");
      chooses[index].setAttribute("aria-selected", "true");
    ...
    select.addEventListener("keydown", (e) => {
      ...
      switch (e.key) {
        case "ArrowDown":
          e.preventDefault();
          currentIndex = (currentIndex + 1) % chooses.length;
          chooses[currentIndex].focus();
          break;
        case "ArrowUp":
          e.preventDefault();
          currentIndex = (currentIndex - 1 + chooses.length) % chooses.length;
          chooses[currentIndex].focus();
          break;
        case "Enter":
        case " ":
...
accordeonTitles.forEach((accordeonTitle) => {
  accordeonTitle.addEventListener("click", () => {
    ...
    const icon = accordeonTitle.querySelector(".accordeon__icon");
    icon.classList.toggle("accordeon__icon--active");
