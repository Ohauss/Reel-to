...
const formValidate = {
  getErrors: function(form) {
    let error = 0;
    const inputs = form.querySelectorAll("[data-fls-input]");
  ...
  addError: function(input) {
    ...
    if (nextEl && nextEl.hasAttribute("data-fls-form-error")) {
      return;
    }
    const errorText = input.getAttribute("data-error") || input.getAttribute("data-fls-form-errtext") || "Error";
    const errorDiv = document.createElement("div");
    errorDiv.setAttribute("data-fls-form-error", "");
  ...
  updateButtonState: function(form) {
    ...
    if (!submitBtn) return;
    let errors = this.getErrors(form);
    const checkbox = form.querySelector("[data-fls-checkbox]");
    if (checkbox && checkbox.hasAttribute("required")) {
      if (!checkbox.checked) {
        errors++;
      }
    }
    submitBtn.disabled = errors > 0;
  }
};
function formInit() {
  function formSubmit() {
      ...
      for (const form of forms) {
        ...
        inputs.forEach((input) => {
          input.addEventListener("input", () => {
            formValidate.updateButtonState(form);
          });
        ...
        form.addEventListener("reset", function(e) {
          ...
          formValidate.updateButtonState(form2);
        });
        formValidate.updateButtonState(form);
      }
    }
